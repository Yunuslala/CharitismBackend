...
module.exports = {
  ...
  UpdateUserTodoStatus: AsyncerrorHandler(async (req, res, next) => {
    ...
    if (!isExist) return next(new ErrorHandler(404, "todo does not exist"));
    ...
    const updateStatus = await TodoModel.findByIdAndUpdate(
      { _id: id },
      { status: true },
      { new: true }
    );
  ...
  removeUserTodo: AsyncerrorHandler(async (req, res, next) => {
    ...
    if (!checkAUthorization)
      return next(
        new ErrorHandler(400, "you are not authorized to delete this task ")
      );

    const updateStatus = await TodoModel.findByIdAndDelete(
      { _id: id },
      { status: true },
      { new: true }
    );
